...
<body>
    ...
    <!-- Seção de Fotos Removida - Fotos disponíveis apenas online -->
    <div class="order-info">
        ...
        <div class="info-grid">
            ...
            @if($order->problems_photos && count($order->problems_photos) > 0)
                <div class="info-item full-width">
                    <label>Problemas Identificados (com registro fotográfico):</label>
                        ...
                        @foreach($order->problems_photos as $index => $problem)
                            <li style="margin-bottom: 3px;">
                                ...
                                @if(isset($problem['photos']) && count($problem['photos']) > 0)
                                    <span style="color: #666; font-size: 10px;">({{ count($problem['photos']) }} foto(s))</span>
                                @endif
                            </li>
                        @endforeach
                    </ul>
                    <small style="color: #666; font-size: 9px;">
                        📷 Fotos disponíveis online: {{ url('/os/photos/' . $order->pdf_hash) }}
                    </small>
                </div>
            @endif
            ...
            @if($order->diagnostic)
                <div
                    class="info-item {{ ($order->deadline || ($order->problems_photos && count($order->problems_photos) > 0)) ? '' : 'full-width' }}">
    ...
    <div class="price-box">
        ...
        @if($discountAmount > 0 && !$order->final_cost)
            <small style="font-size: 11px; opacity: 0.9;">
                (Subtotal: R$ {{ number_format($subtotal, 2, ',', '.') }} - Desconto: R$
